...
def call_pandoc(args):
    ...
    vscode_light = os.path.abspath(os.path.join(os.path.dirname(__file__), "backend/common/utils/assets/vscode_light.theme"))
    if not args.output:
...
def call_handle(args):
    input_path = os.path.abspath(args.input_file)
    template_path = os.path.abspath(args.template)
    command = ["python3", "-m", "backend/common", input_path, "exp"]
...
parser.add_argument(
    ...
    "--template",
    help="template name",
    default="backend/common/ref/exp.docx",
    required=False
)
# 输出文件
parser.add_argument(
